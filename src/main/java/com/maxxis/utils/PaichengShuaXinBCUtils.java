...
@Component
public class PaichengShuaXinBCUtils {
    @Autowired
    private ITestService iTestService;

    private static PaichengShuaXinBCUtils paichengShuaXinBCUtils;

    @PostConstruct
    public void init() {
        paichengShuaXinBCUtils = this;
        paichengShuaXinBCUtils.iTestService = this.iTestService;
    }


    public static void paichengBC() throws Exception {
        //  查询BC已排并且没有再做的(按照number顺序)
        List<Test> all = paichengShuaXinBCUtils.iTestService.findTestByStateNoZuoBC();
//
        //        存放实验项目
...
//            System.out.println(Standard);
            List<TimeTest> listDate = paichengShuaXinBCUtils.iTestService.findDataStandard(Standard, TestItem);
...
//    获取BC设定的时间
        List<WorkTimeBC> works = paichengShuaXinBCUtils.iTestService.findworktimeBC();
...
//                        System.out.println("转换时间为：" + result1a);
                            ...
                            paichengShuaXinBCUtils.iTestService.updateBCExpectedDateUP(numberA, result1a);
...
//                        System.out.println("转换时间为：" + result1a);
        ...
        int spbhOne = paichengShuaXinBCUtils.iTestService.findByspbhOneBC();

        int number = numberMax + 2 + spbhOne;
//            System.out.println(number);
//        去数据库把没有循环到的已经排好的数据变成待排(BC)
        paichengShuaXinBCUtils.iTestService.updateBCDaiPai(number);
