...
import com.maxxis.service.impl.ITestServiceImpl;
...
@Component
public class PaichengSpUtils {
    @Autowired
    private ITestService iTestService;

    private static PaichengSpUtils paichengSpUtils;

    @PostConstruct
    public void init(){
        paichengSpUtils=this;
        paichengSpUtils.iTestService=this.iTestService;
    }


    public static void paichengSP() throws Exception {
            //  查询sp已排(按照number顺序)
        List<Test> all =paichengSpUtils.iTestService.findTestByStateSP();
...
//            System.out.println(Standard);
            List<TimeTest> listDate =paichengSpUtils.iTestService.findDataStandard(Standard,TestItem);
...
//    获取设定的时间
        List<WorkTimeSp> works = paichengSpUtils.iTestService.findworktimeSP();
...
//                        System.out.println("转换时间为：" + result1a);
                        paichengSpUtils.iTestService.updateHSUExpectedDateUPSP(numberA, result1a);
...
//        去数据库把没有循环到的已经排好的数据变成待排(SP)
        paichengSpUtils.iTestService.updateSPnumberDaiPai(number);
