...
@Component
public class PaichengShuaXinSStiffnessUtils {
    @Autowired
    private ITestService iTestService;

    private static PaichengShuaXinSStiffnessUtils paichengShuaXinSStiffnessUtils;

    @PostConstruct
    public void init() {
        paichengShuaXinSStiffnessUtils = this;
        paichengShuaXinSStiffnessUtils.iTestService = this.iTestService;
    }


    public static void paichengSStiffness() throws Exception {
        //  查询SStiffness已排并且没有再做的(按照number顺序)
        List<Test> all = paichengShuaXinSStiffnessUtils.iTestService.findTestByStateNoZuoSStiffness();
//
        //        存放实验项目
...
//            System.out.println(Standard);
            List<TimeTest> listDate = paichengShuaXinSStiffnessUtils.iTestService.findDataStandard(Standard, TestItem);
...
//    获取SStiffness设定的时间
        List<WorkTimeSStiffness> works = paichengShuaXinSStiffnessUtils.iTestService.findworktimeSStiffness();
...
//                        System.out.println("转换时间为：" + result1a);
                            ...
                            paichengShuaXinSStiffnessUtils.iTestService.updateHSUExpectedDateUPSStiffness(numberA, result1a);
...
//                        System.out.println("转换时间为：" + result1a);
        ...
        int spbhOne = paichengShuaXinSStiffnessUtils.iTestService.findByspbhOneSStiffness();

        int number = numberMax + 2 + spbhOne;
//            System.out.println(number);
//        去数据库把没有循环到的已经排好的数据变成待排(SStiffness)
        paichengShuaXinSStiffnessUtils.iTestService.updateSStiffnessnumberDaiPai(number);
