...
@Component
public class PaichengShuaXinSpUtils {
    @Autowired
    private ITestService iTestService;

    private static PaichengShuaXinSpUtils paichengShuaXinSpUtils;

    @PostConstruct
    public void init() {
        paichengShuaXinSpUtils = this;
        paichengShuaXinSpUtils.iTestService = this.iTestService;
    }


    public static void paichengSP() throws Exception {
        //  查询sp已排并且没有再做的(按照number顺序)
        List<Test> all = paichengShuaXinSpUtils.iTestService.findTestByStateNoZuoSP();
//
        //        存放实验项目
...
//            System.out.println(Standard);
            List<TimeTest> listDate = paichengShuaXinSpUtils.iTestService.findDataStandard(Standard, TestItem);
...
//    获取设定的时间
        List<WorkTimeSp> works = paichengShuaXinSpUtils.iTestService.findworktimeSP();
...
//                        System.out.println("转换时间为：" + result1a);
                            ...
                            paichengShuaXinSpUtils.iTestService.updateHSUExpectedDateUPSP(numberA, result1a);
...
//                        System.out.println("转换时间为：" + result1a);
        ...
        int spbhOne = paichengShuaXinSpUtils.iTestService.findByspbhOne();

        int number = numberMax + 2 + spbhOne;
//            System.out.println(number);
//        去数据库把没有循环到的已经排好的数据变成待排(SP)
        paichengShuaXinSpUtils.iTestService.updateSPnumberDaiPai(number);
