...
@Component
public class PaichengShuaXin3DUtils {
    @Autowired
    private ITestService iTestService;

    private static PaichengShuaXin3DUtils paichengShuaXin3DUtils;

    @PostConstruct
    public void init() {
        paichengShuaXin3DUtils = this;
        paichengShuaXin3DUtils.iTestService = this.iTestService;
    }


    public static void paicheng3D() throws Exception {
        //  查询3D已排并且没有再做的(按照number顺序)
        List<Test> all = paichengShuaXin3DUtils.iTestService.findTestByStateNoZuo3D();
//
        //        存放实验项目
...
//            System.out.println(Standard);
            List<TimeTest> listDate = paichengShuaXin3DUtils.iTestService.findDataStandard(Standard, TestItem);
...
//    获取BC设定的时间
        List<WorkTime3D> works = paichengShuaXin3DUtils.iTestService.findworktime3D();
...
//                        System.out.println("转换时间为：" + result1a);
                            ...
                            paichengShuaXin3DUtils.iTestService.update3DExpectedDateUP(numberA, result1a);
...
//                        System.out.println("转换时间为：" + result1a);
        ...
        int spbhOne = paichengShuaXin3DUtils.iTestService.findByspbhOne3D();

        int number = numberMax + 2 + spbhOne;
//            System.out.println(number);
//        去数据库把没有循环到的已经排好的数据变成待排(3D)
        paichengShuaXin3DUtils.iTestService.update3DDaiPai(number);
