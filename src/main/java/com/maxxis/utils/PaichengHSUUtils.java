...
@Component
public class PaichengHSUUtils {
    @Autowired
    private ITestService iTestService;

    private static PaichengHSUUtils paichengHSUUtils;

    @PostConstruct
    public void init(){
        paichengHSUUtils=this;
        paichengHSUUtils.iTestService=this.iTestService;
    }


    public static void paichengHSU() throws Exception {
            //  查询hsu已排(按照number顺序)
        List<Test> all =paichengHSUUtils.iTestService.findTestByState();
...
//            System.out.println(Standard);
            List<TimeTest> listDate =paichengHSUUtils.iTestService.findDataStandard(Standard,TestItem);
...
//    获取设定的时间
        List<WorkTimeHSU> works = paichengHSUUtils.iTestService.findworktime();
...
//                        System.out.println("转换时间为：" + result1a);
                        paichengHSUUtils.iTestService.updateHSUExpectedDateUP(numberA, result1a);
...
//        去数据库把没有循环到的已经排好的数据变成待排(HSU)
        paichengHSUUtils.iTestService.updateHSUDaiPai(number);
