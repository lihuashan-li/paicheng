...
@Component
public class PaichengBCUtils {
    @Autowired
    private ITestService iTestService;

    private static PaichengBCUtils paichengBCUtils;

    @PostConstruct
    public void init(){
        paichengBCUtils=this;
        paichengBCUtils.iTestService=this.iTestService;
    }


    public static void paichengBC() throws Exception {
            //  查询BC已排(按照number顺序)
        List<Test> all =paichengBCUtils.iTestService.findTestByStateBC();
...
//            System.out.println(Standard);
            List<TimeTest> listDate =paichengBCUtils.iTestService.findDataStandard(Standard,TestItem);
...
//    获取设定的时间
        List<WorkTimeBC> works = paichengBCUtils.iTestService.findworktimeBC();
...
//                        System.out.println("转换时间为：" + result1a);
                        paichengBCUtils.iTestService.updateBCExpectedDateUP(numberA, result1a);
...
//        去数据库把没有循环到的已经排好的数据变成待排(BC)
        paichengBCUtils.iTestService.updateBCDaiPai(number);
