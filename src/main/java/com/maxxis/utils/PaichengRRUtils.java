...
@Component
public class PaichengRRUtils {
    @Autowired
    private ITestService iTestService;

    private static PaichengRRUtils paichengRRUtils;

    @PostConstruct
    public void init(){
        paichengRRUtils=this;
        paichengRRUtils.iTestService=this.iTestService;
    }


    public static void paichengRR() throws Exception {
            //  查询sp已排(按照number顺序)
        List<Test> all =paichengRRUtils.iTestService.findTestByStateRR();
...
//            System.out.println(Standard);
            List<TimeTest> listDate =paichengRRUtils.iTestService.findDataStandard(Standard,TestItem);
...
//    获取设定的时间
        List<WorkTimeRR> works = paichengRRUtils.iTestService.findworktimeRR();
...
//                        System.out.println("转换时间为：" + result1a);
                        paichengRRUtils.iTestService.updateHSUExpectedDateUPRR(numberA, result1a);
